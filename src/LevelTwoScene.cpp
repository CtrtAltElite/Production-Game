...
void LevelTwoScene::Start()
{
	...
	SoundManager::Instance().Load("../Assets/audio/LevelMusic/LevelTwo/lost_and_found.mp3", "levelTwoMusic", SoundType::SOUND_MUSIC);
	...
	SetBackground(new Background("../Assets/textures/Levels/2nd_Level.png", "secondLevel"));
	...
	if (obstacleFile.is_open()) {
		...
		while (obstacleFile >> fileName >> textureName) {
			...
			GetTotalObstacles().emplace(std::pair<std::string, Obstacle*>(fileName, temp));
...
void LevelTwoScene::SaveObstaclesToFile()
{
}

void LevelTwoScene::LoadObstaclesToFile()
{
}
